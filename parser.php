...
function handle_img_tag($url)
{
	...
	//$img_tag = '<a href="'.$url.'">&lt;Image link&gt;</a>';
...
function do_smilies($text)
{
	...
	for ($i = 0; $i < $num_smilies; ++$i)
		$text = preg_replace("#(?<=.\W|\W.|^\W)".preg_quote($smiley_text[$i], '#')."(?=.\W|\W.|\W$)#m", '$1<img src="http://forum.'.$_SERVER['HTTP_HOST'].'/img/smilies/'.$smiley_img[$i].'" width="15" height="15" alt="'.substr($smiley_img[$i], 0, strrpos($smiley_img[$i], '.')).'" />$2', $text);
