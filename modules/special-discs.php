...
// Discs with no LibCrypt protection
$discs = $mysqli->query('SELECT * FROM `discs` AS `d` WHERE `d`.`d_media`=1 AND `d`.`d_protection_l`=3'.$queryadd.' ORDER BY `d`.`d_title`,`d`.`d_region`');
echo '<h2>PSX CD with no LibCrypt protection ('.$discs->num_rows.')</h2>';
echo '<div class="textblock"><p>';
while ($disc = $discs->fetch_array()) {
	if (defined('LOGGED')) {
		if (in_array($disc['d_id'], $mydiscsarray))
...
// Discs with no LibCrypt protection (verified)
$discs = $mysqli->query('SELECT * FROM `discs` AS `d` WHERE `d`.`d_media`=1 AND `d`.`d_protection_l`=5'.$queryadd.' ORDER BY `d`.`d_title`,`d`.`d_region`');
echo '<h2>PSX CD with no LibCrypt protection (verified) ('.$discs->num_rows.')</h2>';
echo '<div class="textblock"><p>';
while ($disc = $discs->fetch_array()) {
	if (defined('LOGGED')) {
		if (in_array($disc['d_id'], $mydiscsarray))
